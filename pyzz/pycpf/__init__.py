...
def _generate_second_digit(sequence):
    mapped_values = list(map(lambda x,y: x*y, list(range(11, 2, -1)), sequence))
    digit = reduce(lambda x,y: x + y, mapped_values) \
            + (_generate_first_digit(sequence) * 2)
    digit = abs((digit % 11) - 11)
    return 0 if digit > 9 else digit

def cpf():
    random_sequence = [randint(0,9) for i in range(9)]
    random_sequence.append(_generate_first_digit(random_sequence))
    random_sequence.append(_generate_second_digit(random_sequence))
...
def is_a_valid_cpf(given_cpf):
    if is_a_valid_cpf_format(given_cpf):
        ...
        first_digit = _generate_first_digit(given_cpf[0:-2])
        second_digit = _generate_second_digit(given_cpf[0:-2])
